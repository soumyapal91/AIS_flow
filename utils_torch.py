...
def logGMMpdf_t(xp, mu, Sigma, alpha=None):
    ...
    # logpdf = max_term + torch.logsumexp(logScaledComp - max_term.unsqueeze(0), dim=0)

    logpdf = torch.logsumexp(logScaledComp, dim=0)

    return logpdf.squeeze()


def loggausspdf_target_t(xp, x0, P0, P0inv, logdetP0):
    if xp.ndim == 1:
        xp = xp.unsqueeze(0)
    ...
    if isdiag_t(P0):
        ...
        alpha = torch.mm(P0inv, torch.transpose(y, 0, 1))
        chisq = torch.sum(torch.transpose(alpha, 0, 1) * y, axis=1)
...
def logGMMpdf_target_t(xp, mu, Sigma, alpha, Sigma_inv, Sigma_logdet):
    if xp.ndim == 1:
        xp = xp.unsqueeze(0)
    ...
    for i in range(k):
        logScaledComp[i, :] = loggausspdf_target_t(xp, mu[i], Sigma[:, :, i], Sigma_inv[:, :, i], Sigma_logdet[i]) + torch.log(alpha[i])
...
class NormalizingFlowModel(nn.Module):
    ...
    def __init__(self, args, current_prop):
        ...
        self.flows = nn.ModuleList([RealNVP(dim=args.dim) for _ in range(args.n_layer_nf)]).to(self.device)
        ...
        if self.args.learn_var:
            self.logvar_ = nn.Parameter(2*torch.log(torch.tensor(args.sigma_prop))*torch.ones_like(self.mu_), requires_grad=True).to(self.device)
    ...
    def forward(self, samples):
        if self.args.learn_var:
            var_ = torch.exp(self.logvar_)
            cov_ = torch.stack([torch.diag(var_[i, :]) for i in range(self.args.N)], dim=2)

            samples = samples * torch.sqrt(var_).unsqueeze(1)
        ...
        if self.args.weighting == 'Standard':
            ...
            for n in range(self.args.N):
                if self.args.learn_var:
                    log_proposal[n, :] = loggausspdf_t(samples[n, :, :], self.mu_[n, :], cov_[:, :, n])
                else:
                    log_proposal[n, :] = loggausspdf_t(samples[n, :, :], self.mu_[n, :], torch.from_numpy(self.current_prop.cov[:, :, n]) )

        elif self.args.weighting == 'DM':
            if self.args.learn_var:
                log_proposal = logGMMpdf_t(samples_, self.mu_, cov_).reshape([self.args.N, self.args.K])
            else:
